<div class="modal fade" id="likes<?=$post['id']?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php
                    if (is_array($count_likes)) {
                        foreach($count_likes as $user_like) {
                            $follower_user = getUser($user_like['user_Id']);
                            $follower_btn = '';
                            if (checkFollowStatus($user_like['user_Id'])) {
                                $follower_btn = '<button class="btn btn-sm btn-danger unfollow-btn" data-user-id='.$follower_user['id'].'>Unfollow</button>';
                            } else if ($userData['id'] == $user_like['user_Id']) {
                                $follower_btn = '';
                            } else {
